...
data[[1]] <- read.csv(file.path(basepath, "s0.001-mu-0.3-threads16/lda-progress-mongo-s0.001-mu-0.3-5766.csv"));
legends[[1]] <- "momentum=-0.3";

data[[2]] <- read.csv(file.path(basepath, "s0.001-mu+0.0-threads16/lda-progress-mongo-s0.001-mu0-5769.csv"));
...
pdf("../../figures/v2/expt-6-16-threads-momentum.pdf")

plot(data[[1]]$time, data[[1]]$objective, ylim=c(-48000, -39000), type="n", xlab="Runtime (seconds)", ylab="ELBo", main="LDA (SGD, 16 threads, step size 0.001)");
